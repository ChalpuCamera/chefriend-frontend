...
const mockReviews: ReviewResponse[] = [
  ...
  },
  {
    ...
    spiciness: "덜 맵게",
    price: "1.5만원",
    review: "양도 적당하고 맛있어요. 조금 더 덜 달게 해주시면 좋을 것 같습니다. 매운거 못 먹는데 맵기도 괜찮았어요!!",
  },
  {
    id: 3,
    userName: "송파나부랭이",
    ...
    spiciness: "맵게",
    price: "1만원",
    review: "진짜 맛있는데,, 딱 하나 너무 삼삼해요 조금 더 맵게 할 수 있도록 추가 옵션 만들어주세요!!!",
  },
];

export const reviewApi = {
  // 매장별 리뷰 목록 조회 (추후 백엔드 구현 예정)
  getStoreReviews: async (storeId: number, pageable: Pageable = {}): Promise<ApiResponse<PageResponse<ReviewResponse>>> => {
    const params = {
  ...
  getFoodReviews: async (foodId: number, pageable: Pageable = {}): Promise<ApiResponse<PageResponse<ReviewResponse>>> => {
    const params = {
      page: pageable.page ?? 0,
      size: pageable.size ?? 20,
      sort: pageable.sort ?? ['createdAt,desc'],
  ...
  getReview: async (reviewId: number): Promise<ApiResponse<ReviewResponse | undefined>> => {
    // TODO: 백엔드 API 구현 후 연결
