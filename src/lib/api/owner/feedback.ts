...
export const transformFeedbackToReview = (
  ...
  const profile = tasteProfile || mockTasteProfiles[feedback.userNickname] || {
    spicyLevel: 3,
    mealAmount: 3,
    mealSpending: 3
  };
...
const mockReviews: ReviewDisplayData[] = [
  ...
  },
  {
    ...
    spiciness: "맵게",
    price: "1만원",
    reviewText: "진짜 맛있는데,, 딱 하나 너무 삼삼해요 조금 더 맵게 할 수 있도록 추가 옵션 만들어주세요!!!",
  },
];

export const feedbackApi = {
  // 매장별 피드백 목록 조회
  getStoreFeedbacks: async (storeId: number, pageable: Pageable = {}): Promise<ApiResponse<PageResponse<FeedbackResponse>>> => {
    const params = {
  ...
  // 음식별 피드백 목록 조회
  getFoodFeedbacks: async (foodId: number, pageable: Pageable = {}): Promise<ApiResponse<PageResponse<FeedbackResponse>>> => {
    const params = {
      page: pageable.page ?? 0,
      size: pageable.size ?? 20,
      sort: pageable.sort ?? ['createdAt,desc'],
  ...
  // 피드백 상세 조회
  getFeedback: async (feedbackId: number): Promise<ApiResponse<FeedbackResponse>> => {
    return apiClient.get<ApiResponse<FeedbackResponse>>(`/api/customer-feedback/${feedbackId}`);
  },

  // 피드백 생성
  createFeedback: async (data: FeedbackCreateRequest): Promise<ApiResponse<FeedbackResponse>> => {
    return apiClient.post<ApiResponse<FeedbackResponse>>('/api/customer-feedback', data);
  },

  // 내 피드백 목록 조회 (고객용)
  getMyFeedbacks: async (pageable: Pageable = {}): Promise<ApiResponse<PageResponse<FeedbackResponse>>> => {
    const params = {
      page: pageable.page ?? 0,
      size: pageable.size ?? 20,
      sort: pageable.sort ?? ['createdAt,desc'],
  ...
  generatePresignedUrls: async (data: FeedbackPhotosUploadRequest): Promise<ApiResponse<FeedbackPhotosPresignedUrlResponse>> => {
  ...
  getCustomerTaste: async (): Promise<ApiResponse<CustomerTasteDto>> => {
    return apiClient.get<ApiResponse<CustomerTasteDto>>('/api/user/profile/taste');
  },

  // 고객 취향 정보 수정
  updateCustomerTaste: async (data: CustomerTasteDto): Promise<ApiResponse<CustomerTasteDto>> => {
    return apiClient.put<ApiResponse<CustomerTasteDto>>('/api/user/profile/taste', data);
  },

  // 매장 피드백을 리뷰 형태로 변환하여 조회 (UI 표시용)
  getStoreReviews: async (storeId: number, pageable: Pageable = {}): Promise<ReviewDisplayData[]> => {
    try {
  ...
  getFoodReviews: async (foodId: number, pageable: Pageable = {}): Promise<ReviewDisplayData[]> => {
    try {
